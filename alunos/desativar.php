...
if (isset($_GET['id'])) {
    ...
    $sql = "UPDATE aluno SET aluno_status = 'Inativo' WHERE idaluno = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);

    // Executa a consulta e verifica se foi bem-sucedida
    if ($stmt->execute()) {
        echo "<script>alert('Aluno Desativado com sucesso!'); window.location.href='consulta.php';</script>";
    } else {
        echo "Erro ao Desativar o Aluno.";
    }
} else {
