...
if (isset($_GET['id'])) {
    ...
    $sql = "UPDATE maquina SET maquina_status = 'Ativo' WHERE idmaquina = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);

    // Executa a consulta e verifica se foi bem-sucedida
    if ($stmt->execute()) {
        echo "<script>alert('Máquina ativada com sucesso!'); window.location.href='consulta.php';</script>";
    } else {
        echo "Erro ao ativar Máquina.";
    }
} else {
