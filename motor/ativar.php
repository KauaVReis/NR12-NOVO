...
if (isset($_GET['id'])) {
    ...
    $sql = "UPDATE motor SET motor_status = 'Ativo' WHERE idmotor = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);

    // Executa a consulta e verifica se foi bem-sucedida
    if ($stmt->execute()) {
        echo "<script>alert('Motor ativado com sucesso!'); window.location.href='consulta.php';</script>";
    } else {
        echo "Erro ao Ativar o Motor.";
    }
} else {
